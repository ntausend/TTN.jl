struct ProjTPOSum{N<:AbstractNetwork, T} <: AbstractProjTPO{N,T}
    net::N
    tpos::Vector{TPO}
    ortho_center::Vector{Int64} # tracking the current ortho center

    rg_terms_upwards::Vector{Vector{Vector{Vector{Vector{Op}}}}}
    #rg_id_upwars::Vector{Vector{Vector{T}}}
    environments::Vector{Vector{Vector{Vector{Prod{Op}}}}}
end
...
"""
function ProjTPOSum(ttn::TreeTensorNetwork{N, T}, tpos::Vector{TPO{S}}) where {N,T,S}
    net = network(ttn)
    ...
    rg_flws = map(tpo -> _up_rg_flow(ttn, tpo), tpos)
    rg_flws_up = first.(rg_flws)
    ids_up_rg = last.(rg_flws)

    # build the environments
    envs = map(zip(rg_flws_up, ids_up_rg)) do (rg_flw_up, id_up_rg)
      return _build_environments(ttn, rg_flw_up, id_up_rg)
    end

    return ProjTPOSum{N, T}(net, tpos, vcat(ortho_center(ttn)...), rg_flws_up, envs)
end

ProjectedTensorProductOperator(ttn::TreeTensorNetwork, tpos::Vector{TPO}) = ProjTPOSum(ttn, tpos)

environments(ptpo::ProjTPOSum, pos::Tuple{Int, Int}, ind::Int) = ptpo.environments[ind][pos[1]][pos[2]]
...
function rebuild_environments!(projTPO::ProjTPOSum, ttn::TreeTensorNetwork)
    net = network(projTPO)
    ...
    projTPO.rg_terms_upwards .= rg_flws_up
...
function update_environments!(projTPO::ProjTPOSum, isom::ITensor, pos::Tuple{Int, Int}, pos_final::Tuple{Int, Int})

    # pos_final has to be either a child node or the parent node of pos
    @assert pos_final ∈ vcat(child_nodes(network(projTPO), pos), parent_node(network(projTPO), pos))
    
    
    newEnvs = deepcopy(projTPO.environments)

    for (i, env) in enumerate(projTPO.environments)
        # get the envrionments of the current position
        envs_cur = env[pos[1]][pos[2]]
        envs_target = env[pos_final[1]][pos_final[2]]
        # extract all components comming from below
        terms_filtered = map(envs_cur) do smt
            filter(T -> site(T) != pos_final, terms(smt))
        end
        ...
        # projTPO.environments[i][pos_final[1]][pos_final[2]] = vcat(env_n_onsite, env_n_id..., env_n_int)
        newEnvs[i][pos_final[1]][pos_final[2]] = vcat(env_n_onsite, env_n_id..., env_n_int)
    end

    for (i,_) in enumerate(projTPO.environments)
      projTPO.environments[i][pos_final[1]][pos_final[2]] = newEnvs[i][pos_final[1]][pos_final[2]]
    end
    return projTPO
end
...
"""
function ∂A(projTPO::ProjTPOSum, pos::Tuple{Int,Int}; factors=ones(length(projTPO.environments)))
