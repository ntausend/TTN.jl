...
function _build_domains_and_codomains(net::AbstractNetwork{D,S,I}, target_charge::Union{I, Nothing}, maxdim::Int, tries::Int) where{D,S, I<:Sector}
    if(I == Trivial)
        target_sp = spacetype(net)(1)
    else
        if isnothing(target_charge)
            target_sp = spacetype(net)()
        else
    ...
    for tt in 1:tries
        ...
        for ll in eachlayer(net)
            for pp in eachindex(net, ll)
                ...
                if(isnothing(target_charge) && !(I== Trivial) && ll == number_of_layers(net))
                    domain_new = fuse(codomain_ch...)
                    domains[ll][pp] = _correct_domain(domain_new, 1)
                elseif ll == number_of_layers(net)
                    # correct in case of final layer...
                    domains[ll][pp] = target_sp
                else
        ...
        I == Trivial || isnothing(target_charge) &&  break
